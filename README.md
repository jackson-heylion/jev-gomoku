...
历史棋谱第十一局（黑棋禁长连、四四，允许三三）揭示了一个传统搜索与 Jev 决策之间的安全漏洞：白方第 36 手 `D9` 制造了一个看似主动的单点威胁，但黑棋被迫下 `C9` 后，黑方同时获得了 `D8 → G5 / B10` 与 `G11 → E11 / J11` 两处独立的双端胜点生成手。**第 38 手再堵 `D8` 已经无法同时消除 `G11`**。

新增的 **Forced-defense Fork Exposure** 在分析“我方只有一个立即胜点”的候选时，模拟对手唯一合法封挡手，并检测封挡后是否同时存在两个互不依赖的双端威胁生成点：

- 将风险作为结构化证据提供给 JEV 和决策日志，避免“自己是进攻方，所以忽略对手反击”；
- Jev 提议的 Wildcard 也必须通过相同检查，不能绕过主候选的战术边界；
- 当存在没有同类风险的候选时，Jev Max 在确定性候选过滤中先保留这些候选；
- 并行 Threat / Deep 的证明仍有效，**形状风险不自动升级为数学意义的必败证明**；若所有候选都有风险，则不会凭这个模式删光候选。

保留本局第 36、38、40 手的回归测试，检查提前识别、独立威胁生成点和终局双胜点；真实 Jev 对局胜率仍需另外进行付费历史 A/B 验证。
