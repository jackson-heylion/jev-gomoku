...
/**
 * Historical real-Jev losses showed a narrower failure mode: Jev can override
 * the actual Local #1 with a move that bounded Deep evaluates catastrophically
 * worse. The guard must act only on the final semantic override; it must never
 * pre-filter the whole candidate pool.
 */
async function testHistoricalSemanticOverrideGuard() {
  ...
  if (result.jevSuggested !== 'G10') {
    throw new Error('Regression mock must semantically prefer historical G10, got ' + result.jevSuggested);
  }
  if (result.finalChoice === 'G10') {
    throw new Error('Narrow semantic override guard failed to veto historical G10');
  }
  if (result.finalChoice !== 'F7') {
    throw new Error('Historical G10 guard should fall back to actual Local #1 F7, got ' + result.finalChoice);
  }
  if (!guard?.vetoed || guard.localMove !== 'F7' || guard.semanticMove !== 'G10') {
    throw new Error('Historical G10 override guard diagnostics are incomplete: ' + JSON.stringify(guard));
  }
  if (!String(result.decisionTrace?.requestShape?.decisionAuthority || '').endsWith('_deep_guard')) {
    throw new Error('Semantic override veto must be visible in decisionAuthority');
  }
  if (requests < 1 || requests > 2) {
    throw new Error('Semantic override guard changed Jev request cap: ' + requests);
  }

  // Pure verdict boundary for the recent depth=0 D8 regression:
  // depth 4 is enough only when the separation is huge and Local is not already
  // a hard loss. This tests the decision rule without relying on worker timing.
  position = positionFromSequence([
    'H8','G9','H9','H10','F8','G8','I11','G10','G7','G11','G12','F10','I10','D10','E10',
    'I9','F12','J8','K7','H12','I13'
  ]);
  engine.setPosition(position.board, position.moves, 'jev-latest');
  const verdict = engine.maxSemanticOverrideVerdict('E9', 'D8', {
    status: 'completed',
    depthReached: 4,
    scores: [
      { move: 'E9', score: -1074, forcedResult: null },
      { move: 'D8', score: -36720, forcedResult: null }
    ]
  });
  if (!verdict?.vetoed || verdict.reason !== 'semantic_catastrophic_deep_separation') {
    throw new Error('Depth-4 catastrophic D8 separation must be vetoed: ' + JSON.stringify(verdict));
  }
}
