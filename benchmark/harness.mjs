...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    maxCriticalDoubleThreeTieBreak(keys, threatAnalysis, deepAnalysis) { const wanted = new Set(keys || []); const context = buildAdvancedCandidates('max'); let candidates = context.candidates.filter(move => wanted.has(move.key)); candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); attachMaxDeepEvidence(candidates, deepAnalysis); candidates = hardFilterMaxCandidates(candidates, threatAnalysis); const result = applyCriticalDoubleThreeDeepTieBreak(candidates, deepAnalysis); return { ...result, candidates: result.candidates.map(move => move.key) }; },",
