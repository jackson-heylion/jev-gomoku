...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    maxDeepSafetyFinalist(mode, finalistKeys, deepAnalysis, threatAnalysis) { const context = buildAdvancedCandidates(mode || 'max'); const candidates = context.candidates; candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); attachThreatEvidence(candidates, threatAnalysis || null); attachMaxDeepEvidence(candidates, deepAnalysis || null); const finalists = (finalistKeys || []).map(key => candidates.find(move => move.key === key)).filter(Boolean); const selected = selectMaxDeepSafetyFinalist(candidates, finalists, deepAnalysis || null); return { choice: selected?.key || null, candidates: candidates.map(move => move.key), finalistRisks: finalists.map(move => ({ move: move.key, risk: move.threatSearch?.counterThreat?.risk || null })), selectedRisk: selected?.threatSearch?.counterThreat?.risk || null }; },",
