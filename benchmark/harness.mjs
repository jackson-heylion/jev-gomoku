...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    maxSpeculativePayloadWithDeep(mode, deepAnalysis) { const context = buildAdvancedCandidates(mode || 'max'); const candidates = context.candidates; candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); attachMaxDeepEvidence(candidates, deepAnalysis || null); const pool = candidates.slice(0, Math.min(6, candidates.length)); return buildMaxSpeculativePayload(context, candidates, pool, extraWildcardPool(candidates, 12, new Set())).payload; },",
    "    opponentProfile() { return opponentBehaviorProfile(); },",
    "    forcedDefenseForkRisk(key) { const m = parseCoord(key); const side = aiColor(); if (!m || board[m.r]?.[m.c] !== EMPTY) return null; board[m.r][m.c] = side; try { return forcedDefenseForkExposure(side, otherColor(side), immediateWins(side, 2), 2); } finally { board[m.r][m.c] = EMPTY; } },",
    "    maxWildcardEligibility(key) { const m = parseCoord(key); return m ? validateWildcardCandidate({ ...m, key }) : null; },",
    "    maxDeepDominance(mode, deepAnalysis) { const context = buildAdvancedCandidates(mode || 'max'); const candidates = context.candidates; candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); attachMaxDeepEvidence(candidates, deepAnalysis); const result = applyMaxDeepDominance(candidates, deepAnalysis, context.localSearchChoice || null); return { ...result, localSearchChoice: context.localSearchChoice || null, recallPriorityChoice: context.candidates[0]?.key || null, candidates: result.candidates.map(move => move.key) }; },",
