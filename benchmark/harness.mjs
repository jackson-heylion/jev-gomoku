...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    maxSemanticOverrideVerdict(localKey, semanticKey, deepAnalysis, mode) { const context = buildAdvancedCandidates(mode || 'max'); const localMove = context.candidates.find(move => move.key === localKey) || null; const semanticMove = context.candidates.find(move => move.key === semanticKey) || null; return semanticOverrideDeepVerdict(localMove, semanticMove, deepAnalysis); },",
