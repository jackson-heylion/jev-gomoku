...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    async maxSemanticOverrideGuard(mode, semanticKey) { const engineMode = mode || 'max'; const context = buildAdvancedCandidates(engineMode); const candidates = context.candidates; candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); const initial = await runDeepWorkerVerification(candidates.slice(0, Math.min(5, candidates.length)), engineMode, 'regression_override_guard_initial'); attachMaxDeepEvidence(candidates, initial); const semantic = candidates.find(move => move.key === String(semanticKey || '').toUpperCase()); if (!semantic) throw new Error('Semantic override candidate missing: ' + semanticKey); const result = await runMaxSemanticOverrideGuard(context, candidates, semantic); return { ...result, initial: initial ? { status: initial.status, depthReached: initial.depthReached, timedOut: initial.timedOut, elapsedMs: initial.elapsedMs, scores: (initial.scores || []).map(row => ({ move: row.move, score: row.score, forcedResult: row.forcedResult || null })) } : null, scores: (result.analysis?.scores || []).map(row => ({ move: row.move, score: row.score, forcedResult: row.forcedResult || null })), analysis: result.analysis ? { status: result.analysis.status, depthReached: result.analysis.depthReached, timedOut: result.analysis.timedOut, elapsedMs: result.analysis.elapsedMs } : null }; },",
