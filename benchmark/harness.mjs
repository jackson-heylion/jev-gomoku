...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    async maxSemanticOverrideGuard(mode, semanticKey) { const context = buildAdvancedCandidates(mode || 'max'); const candidates = context.candidates; candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); const semantic = candidates.find(move => move.key === String(semanticKey || '').toUpperCase()); if (!semantic) throw new Error('Semantic override candidate missing: ' + semanticKey); const result = await runMaxSemanticOverrideGuard(context, candidates, semantic); return { ...result, scores: (result.analysis?.scores || []).map(row => ({ move: row.move, score: row.score, forcedResult: row.forcedResult || null })), analysis: result.analysis ? { status: result.analysis.status, depthReached: result.analysis.depthReached, timedOut: result.analysis.timedOut, elapsedMs: result.analysis.elapsedMs } : null }; },",
