...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    maxThreatSelection(mode) { const context = buildAdvancedCandidates(mode || 'max'); return selectMaxThreatCandidates(context.candidates, 6).map(move => ({ move: move.key, sources: [...(move.recallSources || [])], searchScore: move.searchScore ?? null })); },",
