...
function injectBenchmarkHook(source) {
  ...
  const hook = [
    ...
    "    maxDeepDominance(mode, deepAnalysis) { const context = buildAdvancedCandidates(mode || 'max'); const candidates = context.candidates; candidates.forEach((move, index) => { move.localRank = move.rank ?? index + 1; }); attachMaxDeepEvidence(candidates, deepAnalysis); const result = applyMaxDeepDominance(candidates, deepAnalysis, context.candidates[0]?.key || null); return { ...result, candidates: result.candidates.map(move => move.key) }; },",
