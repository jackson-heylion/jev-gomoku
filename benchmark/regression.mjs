...
/**
 * One move after the depth0 D8 regression, production Alpha-Beta prefers D9,
 * but Threat search hard-proves D9 losing. The semantic guard must never
 * resurrect that filtered Local #1 when Jev selects a surviving move.
 */
async function testSemanticOverrideNeverResurrectsHardLostLocal() {
  let requests = 0;
  const engine = await loadProductionEngine({
    request: async ({ payload }) => {
      requests++;
      ...
      for (const [id, question] of Object.entries(payload?.questions || {})) {
        const keys = Object.keys(question?.criteria || {});
        if (!keys.length) throw new Error('Hard-lost-local mock has no choices: ' + id);
        let choice = keys[0];
        if (id === 'recall_check') {
          choice = keys.includes('MAIN_SET') ? 'MAIN_SET' : keys[0];
        } else if (id.startsWith('judge_')) {
  ...
  const d9 = (threat?.analyses || []).find(row => row.move === 'D9');
  if (!d9?.forced) {
    throw new Error('Historical D9 must be hard-proved losing before semantic guard: ' + JSON.stringify(d9 || null));
  }
  ...
  if (result.finalChoice === 'D9') {
    throw new Error('Semantic guard resurrected hard-lost Local #1 D9');
  }
  if (guard?.vetoed && guard.choice === 'D9') {
    throw new Error('Semantic guard vetoed back to filtered D9: ' + JSON.stringify(guard));
  }
  if (requests < 1 || requests > 2) {
